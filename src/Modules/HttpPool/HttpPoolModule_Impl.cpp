...
namespace module
{
	...
	{
		return (module::IHttpPoolModule*)logic::GetLogic()->getModule(MODULE_ID_HTTPPOOL);
	}
}
...
void HttpPoolModule_Impl::release()
{
	delete this;
}
...
{
	TTAutoLock lock(&m_mtxLock);
	auto iter = m_lstHttpOpers.begin();
	while (iter != m_lstHttpOpers.end())
	{
		try
		{
			(*iter)->release();
...
UInt32 TTHttpThread::process()
{
	...
	while (m_bContinue)
	{
		...
		{
			TTAutoLock lock(&(pPool->m_mtxLock));
		...
		{
			APP_LOG(LOG_ERROR, _T("TTHttpThread: Failed to execute opertaion(0x%p)"), pHttpOper);
		}
	}
