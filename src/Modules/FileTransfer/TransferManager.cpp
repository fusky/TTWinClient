...
{
	CString strName;
	strName = util::utf8ToCString(sFileName.c_str());
...
BOOL TransferFileEntityManager::openFileByFileID(IN const std::string& sID)
{
	...
	for (; it != m_VecFinishedFile.end(); ++it)
	{
		if (sID == it->sTaskID)
		{
			...
			if (util::isFileExist(strPath))
			{				
				HINSTANCE hr = ShellExecute(NULL, _T("open"), strPath, NULL, NULL, SW_SHOWNORMAL);
				if ((long)hr < 32)
				{
					DWORD dwLastError = ::GetLastError();
					APP_LOG(LOG_ERROR, _T("FileTransferSevice_Impl::OpenFileByFileID ShellExecute Failed GetLastError = %d")
						, dwLastError);
				}
			}
			return TRUE;
...
BOOL TransferFileEntityManager::checkIfIsSending(IN CString sFilePath)
{
	...
	for (; it != m_MapFile.end(); ++it)
	{
		...
		if ((CLIENT_OFFLINE_UPLOAD == it->second.nClientMode || CLIENT_REALTIME_SENDER == it->second.nClientMode)
			&& sfileName == sFilePath)
		{
			APP_LOG(LOG_DEBUG, _T("FileTransferSevice_Impl::_checkIfIsSending isSending sFileID = %s"), sFilePath);
			return TRUE;
		}
	}
	return FALSE;
