...
,readOnly(TRUE)
{
	...
	if (!file)
		APP_LOG(LOG_INFO,_T("Could not open file %s"), path);
}
...
BOOL TransferFile::writeBlock(UInt32 offset, UInt32 size, void* data)
{
	...
	{
		APP_LOG(LOG_ERROR, _T("TransferFile::writeBlock() -- attempted to write to a file that is not writable"));
		return FALSE;
	}
...
BOOL TransferFile::readBlock(UInt32 offset, UInt32 size, OUT std::string& data)
{
	...
	{
		APP_LOG(LOG_ERROR, _T("TransferFile::readBlock() -- Invalid File"));
		return FALSE;
	}
	seek(offset, Beginning);
	data.reserve(size);
...
void TransferFile::seek(long offset, Position p /*= Beginning*/)
{
	if (!m_pfileObject->file)
	{
		APP_LOG(LOG_ERROR, _T("TransferFile::seek() -- trying to seek in a file that isn't opened."));
		return;
	}
