...
void UIIMEdit::InsertImage(BSTR bstrFileName,SIZE size,BOOL isGif)
{
	...
	{
		APP_LOG(LOG_ERROR, _T("InsertImage CoCreateInstance failed"));
		goto End;
	}
	...
	if (FAILED(hr))
	{
		APP_LOG(LOG_ERROR, _T("InsertImage CreateILockBytesOnHGlobal failed"));
		goto End;
	}
	...
	if (sc != S_OK)
	{
		APP_LOG(LOG_ERROR, _T("InsertImage StgCreateDocfileOnILockBytes failed"));
		goto End;
	}
	...
	try
	{
		...
		if (FAILED(hr))
		{
			APP_LOG(LOG_ERROR, _T("InsertImage lpAnimator QueryInterface failed"));
			goto End;
		}
	...
	{
		APP_LOG(LOG_ERROR, _T("InsertImage unknown exeption"));
	}
...
BOOL UIIMEdit::GetContent(OUT MixedMsg& mixMsg)
{
	...
	{
		APP_LOG(LOG_DEBUG, _T("UIIMEdit::GetContent IsPureTextMsg"));
	}
	else//图文混排
	{
		...
		for (UInt32 i = 0; i < nImageCount; i++)
		{
			...
			if (GetPicPosAndPathbyOrder(i, picData.nPos, picData.strLocalPicPath))
			{
				...
				if (-1 != nPos)
				{
					...
					CString fileID;
					module::getEmotionModule()->getEmotionIDByName(fileName, fileID);
					...
					nPosAdd = picData.nPos + fileID.GetLength() - 1;					
				}
				else
				{
