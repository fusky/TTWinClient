...
Node* CUIRecentSessionList::AddNode(const SessionListItemInfo& item, Node* parent /*= NULL*/, int index /*= 0*/)
{
	...
	CLabelUI* lastContent = static_cast<CLabelUI*>(paint_manager_.FindSubControlByName(pListElement, lastContentTimeControlName));
	if (lastContent != NULL)
	{
...
BOOL CUIRecentSessionList::AddNode(const std::string& sId)
{
	...
	{
		APP_LOG(LOG_DEBUG, _T("CUISessionList::AddNode,already exist!"));
		return FALSE;
	}
	...
	if (pSessionInfo->m_sessionType == SESSION_USERTYPE)
	{
		...
		{
			APP_LOG(LOG_DEBUG, _T("CUISessionList::AddNode,can't find the UserInfoEntity!"));
			return FALSE;
		}
	...
	else if (SESSION_GROUPTYPE == pSessionInfo->m_sessionType)
	{
		module::GroupInfoEntity groupInfo;
		if (!module::getGroupListModule()->getGroupInfoBySId(sId, groupInfo))
		{
			APP_LOG(LOG_DEBUG, _T("CUISessionList::AddNode,can't find the GroupInfoEntity!"));
			return FALSE;
		}
...
int CALLBACK IMListItemCompareFunc(UINT_PTR pa, UINT_PTR pb, UINT_PTR pUser)
{
	...
	SessionEntity* psEntity1 = SessionEntityManager::getInstance()->getSessionEntityBySId(sid1);
	SessionEntity* psEntity2 = SessionEntityManager::getInstance()->getSessionEntityBySId(sid2);
	
	if (!psEntity1 || !psEntity2)
	{
		return 0;
	}
	time_t m_updatedTime1 = psEntity1->m_updatedTime;
	time_t m_updatedTime2 = psEntity2->m_updatedTime;
	...
	int  nRes = psEntity1->m_updatedTime > psEntity2->m_updatedTime ? -1 : 1;
	return nRes;
}
...
BOOL CUIRecentSessionList::UpdateItemBySId(const std::string& sId)
{
	if (sId.empty())
	{
		return FALSE;
	...
	//更新最近一条未读消息
	MessageEntity msg;
	if (!ReceiveMsgManage::getInstance()->frontMessageBySId(sId, msg))
	{
		//若没有未读消息，则需要更新最近会话的时间-本地发出去的最后一条消息
		SessionEntity*  pSessionEntity = SessionEntityManager::getInstance()->getSessionEntityBySId(sId);
		if (pSessionEntity)
		{
			CString strTime = module::getMiscModule()->makeShortTimeDescription(static_cast<UInt32>(pSessionEntity->m_updatedTime));
	...
	if (MESSAGE_RENDERTYPE_AUDIO == msg.msgRenderType)
	{
		strContent = util::getMultilingual()->getStringViaID(_T("STRID_SESSIONMODULE_RENDERTYPE_AUDIO"));
	}
	else
	{
		strContent = util::stringToCString(msg.content);
		ReceiveMsgManage::getInstance()->parseContent(strContent, TRUE, 400, FALSE);
	}
	...
	//更新消息的时间
	CString strTime = module::getMiscModule()->makeShortTimeDescription(msg.msgTime);
	plastMsgTimeUI->SetText(strTime);
