		...
		if (child && child->folder())
		{
			...
			if (strID == sId)
			{
				...
				if (plistElmen)
				{
					...
					{
						CString strReplace;
						strReplace.Format(_T("[%d]"), retNode->num_children() + 1);
						int nPos = strParentName.ReverseFind('[');
						strParentName = strParentName.Mid(0, nPos);
						strParentName += strReplace;
...
void UIIMList::_FreshAvatarBySId(const std::string& sId)
{
	...
	CLabelUI* nick_name = static_cast<CLabelUI*>(paint_manager_.FindSubControlByName(pListElement, kNickNameControlName));
	PTR_VOID(nick_name);
	user.isOnlne() ? nick_name->SetTextColor(RGB(0, 0, 0)) : nick_name->SetTextColor(RGB(153, 153, 153));
}
