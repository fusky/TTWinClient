...
GetServConfigHttpOperation::GetServConfigHttpOperation(logic::ICallbackHandler& callback, GetServConfigParam& param)
...
void GetServConfigHttpOperation::process()
{
	...
	if (!client.execute(&request, &response))
	{
		...
		APP_LOG(LOG_ERROR,_T("GetServConfigHttpOperation failed %s"), csTemp);
	...
	try
	{
		...
		if (!reader.parse(body, root))
		{
			...
			APP_LOG(LOG_ERROR, _T("GetServConfigHttpOperation parse data failed,%s"), csTemp);
			pPamram->result = GETSERV_CONFIT_FAIL;
			goto End;
		}
		...
		{
			CString csTemp = util::stringToCString(body);
			APP_LOG(LOG_ERROR, _T("GetServConfigHttpOperation parse login IP failed,%s"), csTemp);
			pPamram->result = GETSERV_CONFIT_FAIL;
			goto End;
		}
	...
	{
		CString csTemp = util::stringToCString(body);
		APP_LOG(LOG_ERROR,_T("GetServConfigHttpOperation parse json execption,%s"), csTemp);
		pPamram->result = GETSERV_CONFIT_FAIL;
	}
