...
{
	APP_LOG(LOG_ERROR, _T("_InitlizeSetting,读取设置失败"));
...
void SysConfigDialog::_ApplySetting()
{
	...
	if (!pTTConfig)
	{
		APP_LOG(LOG_ERROR, _T("SysConfigDialog::_ApplySetting,读取设置失败"));
		return;
	}
	...
	logic::GetLogic()->asynNotifyObserver(module::KEY_SYSCONFIG_UPDATED);	//通知改动
