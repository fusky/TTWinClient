...
//				  * New syntax: FastDelegate< int (TCHAR *, double) >. 
...
//     (TCHAR *) first to ensure that the correct number of *bytes* are added.
//
////////////////////////////////////////////////////////////////////////////////
//						Helper templates
//
...
struct SimplifyMemFunc {
	...
	inline static GenericClass *Convert(X *pthis, XFuncType function_to_bind, 
		...
		typedef TCHAR ERROR_Unsupported_member_function_pointer_on_this_compiler[N-100];
		return 0; 
	}
};
...
struct SimplifyMemFunc< SINGLE_MEMFUNCPTR_SIZE + sizeof(int) >  {
	...
	inline static GenericClass *Convert(X *pthis, XFuncType function_to_bind, 
		...
		return reinterpret_cast<GenericClass *>(reinterpret_cast<TCHAR *>(pthis) + u.s.delta); 
	}
};
...
struct SimplifyMemFunc<SINGLE_MEMFUNCPTR_SIZE + 3*sizeof(int) >
{
	...
	inline static GenericClass *Convert(X *pthis, XFuncType function_to_bind, 
		...
		typedef TCHAR ERROR_VC6CompilerBug[-100];
		return 0; 
	}
};
	...
	inline static GenericClass *Convert(X *pthis, XFuncType function_to_bind, 
		...
		if (u.s.vtable_index) { // Virtual inheritance is used
			...
			const int * vtable = *reinterpret_cast<const int *const*>(
				reinterpret_cast<const TCHAR *>(pthis) + u.s.vtordisp );

			// 'vtable_index' tells us where in the table we should be looking.
			virtual_delta = u.s.vtordisp + *reinterpret_cast<const int *>( 
				reinterpret_cast<const TCHAR *>(vtable) + u.s.vtable_index);
		}
		// The int at 'virtual_delta' gives us the amount to add to 'this'.
        // Finally we can add the three components together. Phew!
        return reinterpret_cast<GenericClass *>(
			reinterpret_cast<TCHAR *>(pthis) + u.s.delta + virtual_delta);
	};
};
...
//   FastDelegate3<int, TCHAR *, double>
