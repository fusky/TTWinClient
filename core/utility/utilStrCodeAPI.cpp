...
CString int64ToCString(Int64 n)
{
	CString	 s;
	s.Format(_T("%I64d"), n);
	return s;
}
Int64	cstringToInt64(LPCTSTR buff)
{
	return _tstol(buff);
}
...
CString utf8ToCString(const char* src)
{
	...
	WCHAR* pBuffer = dst.GetBufferSetLength(length - 1);
	::MultiByteToWideChar(CP_UTF8, 0, src, -1, pBuffer, length);
...
{
	std::string dst;
	int length = ::WideCharToMultiByte(CP_UTF8, 0, src, src.GetLength(), NULL, 0, NULL, NULL);
	dst.resize(length);
	::WideCharToMultiByte(CP_UTF8, 0, src, src.GetLength(), &dst[0], (int)dst.size(), NULL, NULL);
