...
CRichEditUI::CRichEditUI() : m_pTwh(NULL), m_bVScrollBarFixing(false), m_bWantTab(true), m_bWantReturn(true), 
	...
	m_iLimitText(cInitTextMax), m_lTwhStyle(ES_MULTILINE), m_bInited(false), m_chLeadByte(0), m_pCallback(NULL), m_pRichEditOle(NULL)
{
...
void CRichEditUI::DoEvent(TEventUI& event)
{
    ...
    if( event.Type == UIEVENT_SETFOCUS ) {
        if( m_pTwh ) {
			//OutputDebugString(_T("m_pTwh---UIEVENT_SETFOCUS\r\n"));
            m_pTwh->OnTxInPlaceActivate(NULL);
